...
Approximately _N_ number of  HLSL shaders will be generated. These shaders use fixed input buffers and write results to output buffers, where randomness is derived from branching logic.
...
### Execution Pipeline
Only the code for the **random shader generator** will reside in the offload-test-suite repository.
...
It is worth noting that failures may originate from driver compilers rather than the frontend compilers.
...
This approach involves generating all shaders offline and storing them in the repository. Although this is a straightforward implementation, it's not necessary to maintain physical copies of the random shaders. We may later want to change the parameters of the generator (e.g. seed, nesting level).
